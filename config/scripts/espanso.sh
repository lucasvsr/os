...
sudo useradd --system --create-home ${CARGO_USER} && echo "$CARGO_USER ALL=(ALL:ALL) NOPASSWD:ALL" | tee /etc/sudoers.d/${CARGO_USER}
sudo su - ${CARGO_USER}
sudo cargo install --force cargo-make --version 0.34.0

git clone https://github.com/federico-terzi/espanso /tmp/espanso

cd /tmp/espanso
sudo cargo make --profile release --env NO_X11=true build-binary 
